...
const displayCountry = async function () {
  try {
    ...
    if (!allCountrysRes.ok) throw new Error("Problem getting Countrys Data");
...
btnSearchEl.addEventListener("click", function (e) {
  ...
  const country = allCountrysData.find(
    (country) => country.name.common === searchedCountry
  );
...
regionEl.addEventListener("change", function (e) {
  if (e.target.value === "allCountry") {
    ...
    const countrysByRegion = allCountrysData.filter(
      (country) => country.region === clickedZone
    );
